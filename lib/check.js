      ...
      doQuery(query, function(translations) {
        _.each(translations, function(trans) {
          if(trans.approved) {
            ...
            isSecureHTML || report.push({ translation: trans, type: 'tags' });

            var isSameHandlebars = hasSameHandlebars(org, trans);
            isSameHandlebars || report.push({ translation: trans, type: 'handlebars' });

            var isTrimmed = trans.translation && trans.translation === trans.translation.trim();
            isTrimmed || report.push({ translation: trans, type: 'trim' });

            if(!isSecureHTML || !isSameHandlebars || !isTrimmed) {
              totalTasks++;
...
function showReport() {
  ...
  _.each(report, function(trouble) {
    ...
    switch(trouble.type) {
      case 'tags': console.log('Error: [' + t.locale + '] ' + t.original + ' has some insecure tags'); break;
      case 'handlebars': console.log('Warning: [' + t.locale + '] ' + t.original + ' has some handlebars differences'); break;
      case 'trim': console.log('Warning: [' + t.locale + '] ' + t.original + ' is not trimmed'); break;
    }
  });
